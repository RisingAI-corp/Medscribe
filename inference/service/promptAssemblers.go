...
const (
	baseSystemPrompt = `
You are an AI medical assistant acting on behalf of the provider.
Your core responsibility is to generate individual sections of a clinical visit report with strict factual accuracy and adherence to the Task Instructions provided below.
This documentation may be subject to clinical review and legal scrutiny.
Your output must be complete, structured, medically appropriate, and fully aligned with the transcript content.
`

	defaultReturnFormatSystemPrompt = `
Return Format (Always Follow These Rules):
Responses must be returned strictly as plain text with no markdown characters (e.g., '*', '-', '#', or '---').
Responses must be standalone, suitable for direct display inside a text box without client-side cleanup.
Do not phrase responses as answers to questions or in a conversational tone — generate only the requested section of the report.
Do not reference the fact that this is part of an iterative or composite system; output should appear self-contained and final.
If the transcript lacks sufficient information for this section, omit it silently. Do not insert placeholders like 'N/A' or 'Unable to determine'.
`

	defaultWarningsSystemPrompt = `
Warnings:
Your output must be plain text only — absolutely no markdown or HTML formatting.
You must rely exclusively on the transcript provided for this session.
Do not fabricate, speculate, or include knowledge from prior sessions or outside data sources.
`

	userGenerateReportPromptTemplate = `
...
func GenerateReportContentPrompt(cfg generatePromptConfig) string {
    ...
    // --- Prompt Construction ---
    prompt := fmt.Sprintf(userGenerateReportPromptTemplate, 
	cfg.patientName, cfg.providerName, cfg.targetSection, cfg.transcript, cfg.targetSection, cfg.targetSection, cfg.targetSection)
...
func RegenerateReportContentPrompt(cfg regeneratePromptConfig) string {
	...
	// --- Default Formatting and Warnings ---
	prompt += defaultReturnFormatSystemPrompt + "\n\n" + defaultWarningsSystemPrompt
	return prompt
}
