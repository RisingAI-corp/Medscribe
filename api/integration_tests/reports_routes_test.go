...
func TestReportGenerationRoutesSuccess(t *testing.T) {
	...
	metadata := map[string]interface{}{
		...
		"reportContents": []inferenceService.ReportContentSection{
			...
			{
				ContentType: reports.Objective,
				Content:     "test objective content",
			},
			{
				ContentType: reports.Assessment,
				Content:     "test assessment content",
			},
			{
				ContentType: reports.Planning,
				Content:     "test planning content",
			},
			{
				ContentType: reports.Summary,
	...
	expectedContentTypes := []string{"_id", reports.Subjective, reports.Objective, reports.Assessment, reports.Planning, reports.Summary, reports.FinishedGenerating}
	...
	assert.NotEmpty(t, report.Assessment.Data, "Assessment data should not be empty")
	assert.NotEmpty(t, report.Planning.Data, "Planning data should not be empty")
...
func TestReportGenerationRoutesFailures(t *testing.T) {
	...
	t.Run("should return bad request when content types are invalid", func(t *testing.T) {
		...
		assert.Contains(t, rr.Body.String(), "error generating report")
	})
	...
	t.Run("should successfully regenerate report", func(t *testing.T) {
		body := map[string]interface{}{
			...
			"ReportContents": []map[string]interface{}{
				...
				{
					"ContentType": reports.Planning,
					"Content":     "updated planning content",
				},
				{
					"ContentType": reports.Summary,
		...
		expectedContentTypes := []string{reports.Subjective, reports.Objective, reports.Assessment, reports.Planning, reports.Summary, reports.FinishedGenerating}
		assert.ElementsMatch(t, expectedContentTypes, fields, "expected content types do not match the fields")
		assert.True(t, !hasEmptyValue)
...
func TestUpdateContentSection(t *testing.T) {
	...
	t.Run("should successfully update report", func(t *testing.T) {
		...
		req := httptest.NewRequest(http.MethodPatch, "/report/update", bytes.NewBuffer(body))
		req, err = testEnv.GenerateJWT(req, userID)
		require.NoError(t, err)
		...
		report, err := testEnv.GetTestReport(reportID)
		require.NoError(t, err)
		assert.Equal(t, "HE", report.Pronouns)
	})
...
func TestLearnStyle(t *testing.T) {
	...
	t.Run("should return internal server error when learning style fails", func(t *testing.T) {
		...
		learnStyleRequest := reportsHandler.LearnStyleRequest{
			ReportID:       reportID,
			ContentSection: reports.Subjective,
			Content:        "", // doesn't accept empty strings
		}
